...
const domains = [
  ...
  },
  {
    ...
    tag: "Early Access",
    tagColor: "border-blue-200/70 bg-blue-50/60 text-blue-700",
    href: "/agents/aec",
  ...
  },
  {
    ...
    tag: "Roadmap",
    tagColor: "border-slate-200/70 bg-slate-50 text-slate-500",
    href: "#contact",
    cta: "Talk to us",
  },
] as const;
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        return (
          <g key={i}>
            ...
            {/* title lines */}
            <rect x={x + 20} y={y + 5} width={18} height={2.5} rx="1" fill={done ? "#93c5fd" : "#cbd5e1"} />
...
function AECIllustration() {
  return (
    <svg viewBox="0 0 160 80" fill="none" className="w-full" aria-hidden>
      ...
      {/* Building outline */}
      <rect x="30" y="18" width="100" height="52" rx="2" fill="#eff6ff" stroke="#2563eb" strokeWidth="1.2" />
      {/* Floors */}
      <line x1="30" y1="35" x2="130" y2="35" stroke="#93c5fd" strokeWidth="0.8" />
      <line x1="30" y1="52" x2="130" y2="52" stroke="#93c5fd" strokeWidth="0.8" />
      {/* Columns */}
      <line x1="63" y1="18" x2="63" y2="70" stroke="#93c5fd" strokeWidth="0.8" />
      <line x1="97" y1="18" x2="97" y2="70" stroke="#93c5fd" strokeWidth="0.8" />
      {/* Door */}
      <rect x="72" y="54" width="16" height="16" rx="1" fill="#bfdbfe" stroke="#2563eb" strokeWidth="0.8" />
      {/* Dimension arrows */}
      <line x1="15" y1="18" x2="15" y2="70" stroke="#0ea5e9" strokeWidth="0.8" markerEnd="url(#ar)" markerStart="url(#ar)" />
      <text x="11" y="47" fill="#0ea5e9" fontSize="4.5" fontWeight="600" textAnchor="middle">12m</text>
      {/* AI tag */}
      <rect x="104" y="8" width="42" height="12" rx="3" fill="#2563eb" />
      ...
      <defs>
        <marker id="ar" markerWidth="4" markerHeight="4" refX="2" refY="2" orient="auto">
          <path d="M0,0 L4,2 L0,4" fill="none" stroke="#0ea5e9" strokeWidth="0.8" />
        </marker>
      </defs>
    </svg>
  );
...
function YourDomainIllustration() {
  return (
    <svg viewBox="0 0 160 80" fill="none" className="w-full" aria-hidden>
      {/* Central node */}
      <circle cx="80" cy="40" r="14" fill="#f0f9ff" stroke="#0ea5e9" strokeWidth="1.2" />
      ...
      {/* Pulse ring */}
      <circle cx="80" cy="40" r="20" stroke="#0ea5e9" strokeWidth="0.6" strokeDasharray="3 3" opacity="0.5" />
      <defs>
...
export function UseCases() {
  ...
  return (
    <section className="border-t border-slate-200/60 gradient-surface py-20 sm:py-24" id="domains">
      <Container>
        <Reveal>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {domains.map((d, i) => {
            ...
            return (
              ...
              >
                {/* Illustration */}
                <div className="mb-4 overflow-hidden rounded-xl border border-slate-100 bg-slate-50/60 p-3">
                  <Illustration />
                </div>

                <div className="flex items-start justify-between gap-3">
                  <span className="font-display text-2xl font-semibold text-slate-200 transition-colors group-hover:text-blue-100">
                    {d.n}
                  </span>
                  <LiveBadge label={d.tag} color={d.tagColor} />
                </div>
                <h3 className="mt-2 text-lg font-semibold text-slate-900">{d.t}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-slate-600">{d.p}</p>
                <Link
                  href={d.href}
                  className="mt-5 inline-flex items-center gap-1 text-xs font-medium text-blue-600 transition hover:text-blue-800 hover:gap-2"
                >
                  {d.cta}
