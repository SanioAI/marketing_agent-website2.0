...
export function ProcessingStatus({
  ...
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-2xl border border-indigo-100 bg-indigo-50/40 p-5"
    >
      <div className="flex items-center gap-3">
        <span
          className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-indigo-200/80 bg-white"
          aria-hidden
        >
      ...
      <ul className="mt-4 space-y-2 border-t border-indigo-100/80 pt-4">
        {steps.map((s, i) => (
          <li key={s.key} className="flex items-center gap-2 text-xs">
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                i < idx ? "bg-emerald-500" : i === idx ? "bg-indigo-500" : "bg-slate-300"
              }`}
            />
